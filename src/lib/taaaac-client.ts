/**
 * Taaaac Core Client - Sincronizzazione Feature Flags & Configurazione Tenant per Taskly
 */
...
const DEFAULT_DEV_CONFIG: TenantConfig = {
  id: "dev-tenant",
  nomeAttivita: "Pronto Intervento Taskly (Dev)",
  sottodominio: "taskly",
  modulo: "taskly",
  piano: "PRO",
  stato: "ATTIVO",
...
export function resolveTenantDomain(hostHeader?: string | null): string {
  ...
  return "sos-artigiani.taaaac.eu";
}
...
export async function fetchTenantConfig(domain?: string): Promise<TenantConfig> {
  ...
  return {
    id: "offline-tenant",
    nomeAttivita: "Pronto Intervento Artigiani",
