...
const baseNavLinks = [
  ...
  { href: "/dashboard/interventi", label: "Interventi & Ticket", icon: Wrench },
  { href: "/dashboard/calendario", label: "Pianificazione & Slot", icon: CalendarDays },
  { href: "/dashboard/rapportini", label: "Rapportini & Firme", icon: FileCheck2 },
  { href: "/dashboard/attivita", label: "Attività & To-Do", icon: CheckSquare },
  { href: "/dashboard/tecnici", label: "Tecnici & Mezzi", icon: Users },
  { href: "/dashboard/servizi", label: "Listino Interventi", icon: Layers },
  { href: "/dashboard/clienti", label: "Anagrafica Clienti", icon: UserCheck },
];
    ...
    <>
      ...
      >
        ...
        <div className="px-3 py-2.5 mx-3 my-3 bg-slate-800/80 rounded-2xl border border-slate-700/60 flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-600 to-blue-500 text-white font-black flex items-center justify-center shrink-0 shadow-xs">
            🔧
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-xs font-bold text-white truncate">
              {config?.nomeAttivita || "Taskly Service & Maintenance"}
            </p>
            <p className="text-[10px] text-slate-400 font-mono">Assistenza & Impianti</p>
          </div>
        </div>
          ...
          {activeAddons.length > 0 && (
            <div className="pt-4 pb-1 px-3">
              <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center justify-between mb-1">
                ...
                <span className="text-indigo-400 font-extrabold text-[9px] bg-slate-800 border border-slate-700 px-1.5 py-0.2 rounded-full">
                  ⚡ Taaaac
                </span>
              </div>
                ...
                {activeAddons.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[9px] font-bold px-1.5 py-0.2 rounded-md bg-slate-800 text-indigo-300">
                        {item.badge}
                      </span>
                    </Link>
                  );
